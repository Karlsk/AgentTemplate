...
async function fetchData() {
  ...
  try {
    ...
    const result = await nl2sqlService.listTrainingData(props.instanceId, 'doc', offset, pageSize.value)
...
function confirmDelete(item: TrainingDataListItem) {
  dialog.warning({
    ...
    content: 'Are you sure you want to delete this documentation entry?',
    positiveText: 'Delete',
    negativeText: 'Cancel',
...
<template>
  <div class="py-4">
    ...
    <NSpin :show="loading">
      <NEmpty v-if="!loading && items.length === 0" description="No documentation yet. Add business glossary, terminology, or domain knowledge." />
      <template v-else>
        <NDataTable
          :columns="columns"
          :data="items"
