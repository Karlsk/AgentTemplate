...
if [ -f "$ENV_FILE" ]; then
  ...
  APP_ENV=$ENV docker compose --env-file "$ENV_FILE" up -d --build
else
  APP_ENV=$ENV docker compose up -d --build
fi

echo ""
echo "Core services (db, app) started successfully."
echo "To start monitoring services (prometheus, grafana, cadvisor), run:"
