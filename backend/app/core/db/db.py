...
from common.utils.utils import TerraLogUtil, equals_ignore_case
...
try:
    if os.path.exists(settings.ORACLE_CLIENT_PATH):
        ...
        TerraLogUtil.info("init oracle client success, use thick mode")
    else:
        TerraLogUtil.info("init oracle client failed, because not found oracle client, use thin mode")
except Exception as e:
    TerraLogUtil.error("init oracle client failed, check your client is installed, use thin mode")
    ...
    if db.connect_type == ConnectType.sqlalchemy:
        ...
        try:
            with conn.connect() as connection:
                TerraLogUtil.info("success")
                return True
        except Exception as e:
            TerraLogUtil.error(f"Datasource {ds.id} connection failed: {e}")
            if is_raise:
