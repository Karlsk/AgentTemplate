...
from app.core.llm.openai.llm import BaseChatOpenAI
        ...
        if db_model.config:
            try:
                ...
                additional_params = {item["key"]: prepare_model_arg(
                    item.get('val')) for item in config_raw if "key" in item and "val" in item}
            except Exception:
                pass
...
async def get_default_embedding_config():
    ...
    with Session(engine) as session:
        db_model: AiModelDetail = session.exec(
            select(AiModelDetail).where(
                AiModelDetail.llm_type == 'embedding'
            ).order_by(AiModelDetail.created_at.desc()).first()
        )
        if not db_model:
            raise Exception("No embedding model found in the system")
...
async def get_backup_config(llm_type: str = 'chat'):
    """获取指定类型的备份模型配置"""
    with Session(engine) as session:
        # 获取指定类型的备份模型
        backup_model: AiModelDetail = session.exec(
            select(AiModelDetail).where(
                AiModelDetail.backup_model == True,
                AiModelDetail.llm_type == llm_type,
        ...
        additional_params = {}
        if backup_model.config:
            try:
                config_raw = json.loads(backup_model.config)
                additional_params = {item["key"]: prepare_model_arg(
                    item.get('val')) for item in config_raw if "key" in item and "val" in item}
            except Exception:
                pass
        if not backup_model.api_domain.startswith("http"):
            backup_model.api_domain = base64_decrypt(backup_model.api_domain)
            if backup_model.api_key:
                backup_model.api_key = base64_decrypt(backup_model.api_key)

        # 构造 LLMConfig
        return LLMConfig(
            model_id=backup_model.id,
            model_type="openai" if backup_model.protocol == 1 else "vllm",
            model_name=backup_model.base_model,
            api_key=backup_model.api_key,
            api_base_url=backup_model.api_domain,
            additional_params=additional_params,
        )
